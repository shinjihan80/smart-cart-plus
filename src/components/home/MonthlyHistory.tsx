          ...
          {MONTHLY_DATA.map((m) => {
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm font-medium ${isActive ? 'text-brand-primary' : 'text-gray-400'}`}>
                  {m.label}
                </span>
              </button>
            );
          ...
          {data.orders.map((order) => (
            <div key={order.id} className="flex items-center gap-3">
              <div className={`w-7 h-7 rounded-full ${order.mallBg} flex items-center justify-center shrink-0`}>
                <span className="text-white text-xs font-bold">{order.store.charAt(0)}</span>
              </div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-800 truncate">{order.name}</p>
                <p className="text-sm text-gray-400">{order.store} · {order.date}</p>
              </div>
