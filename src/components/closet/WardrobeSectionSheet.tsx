...
import { WARDROBE_SECTION_META, type WardrobeSection, type WardrobeCell } from '@/lib/wardrobeModel';
...
interface WardrobeSectionSheetProps {
  section:        WardrobeSection | null;
  items:          ClothingItem[];
  getMatchBadge?: (item: ClothingItem) => MatchBadge | undefined;
  onClose:        () => void;
...
export function WardrobeSectionSheet({
  ...
  return (
    <AnimatePresence>
      {section && (() => {
        const meta = WARDROBE_SECTION_META[section];
        return (
          <motion.div
            key={section}
            initial={{ opacity: 0 }}
            ...
            >
              ...
              {items.length === 0 ? (
                <div className="text-center py-12 text-gray-400">
                  ...
                  <p className="text-xs mt-1 leading-relaxed">{meta.hint} 의류를 추가해보세요</p>
                </div>
              ) : (
                <div className="flex flex-col gap-3">
                  {items.map((item, index) => (
                    <SwipeClothingCard
                      ...
                      item={item}
                      index={index}
                      onRemove={(id) => { onRemove(id); setExpandedId(null); }}
                      onUpdate={onUpdate}
                      matchBadge={getMatchBadge?.(item)}
