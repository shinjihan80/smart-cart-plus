  ...
  return (
    ...
    >
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="text-[10px] text-gray-400">· 지금은 {season}</span>
      </div>
        ...
        <div className="rounded-2xl bg-brand-success/5 border border-brand-success/20 px-3 py-2.5 mb-3">
          <div className="flex items-center justify-between gap-2">
            ...
            <button
              ...
              className="shrink-0 text-[11px] font-semibold px-3 py-1.5 rounded-full bg-brand-success text-white hover:opacity-90 transition-opacity"
            >
              모두 꺼내기
            </button>
          </div>
                  ...
                  <div className="flex flex-col gap-1.5">
                    {stowCandidates.slice(0, 10).map((c) => (
                      <div key={c.id} className="flex items-center gap-2 py-1">
                        ...
                        <span className="text-[9px] text-gray-400">{c.weatherTags?.join(', ')}</span>
                        <button
                          onClick={() => handleStow(c)}
                          className="shrink-0 text-[10px] text-brand-primary font-medium hover:underline"
                        >
                          보관
                        </button>
                      </div>
                    ...
                    {stowCandidates.length > 10 && (
                      <p className="text-[9px] text-gray-400 text-center mt-1">외 {stowCandidates.length - 10}벌</p>
                    )}
                  </div>
                </div>
              </motion.div>
            ...
            {unstowOpen && (
              ...
              >
                <div className="px-3 pb-3 flex flex-col gap-1.5">
                  {allStored.slice(0, 12).map((c) => (
                    <div key={c.id} className="flex items-center gap-2 py-1">
                      ...
                      <button
                        ...
                        className="shrink-0 text-[10px] text-brand-success font-medium hover:underline"
                      >
                        꺼내기
                      </button>
                    </div>
                  ...
                  {allStored.length > 12 && (
                    <p className="text-[9px] text-gray-400 text-center mt-1">외 {allStored.length - 12}벌</p>
                  )}
                </div>
              </motion.div>
            )}
