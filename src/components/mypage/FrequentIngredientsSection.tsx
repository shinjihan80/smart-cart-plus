        ...
        {ranked.map((r, i) => {
          ...
          return (
            <div key={r.name} className="flex items-center gap-2 py-1">
              <span className="text-[10px] text-gray-300 tabular-nums w-4 text-right shrink-0">{i + 1}</span>
              <span className="text-sm shrink-0">{getFoodEmoji(r.name)}</span>
              <span className="flex-1 text-sm text-gray-800 truncate">{r.name}</span>
              <span className="text-[10px] text-gray-400 font-semibold tabular-nums shrink-0">{r.count}회</span>
              {owned ? (
                <span className="text-[9px] text-brand-success font-medium shrink-0">✓ 보유</span>
              ) : (
                <button
