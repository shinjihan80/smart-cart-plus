        ...
        >
          ...
          <div className={`rounded-[28px] bg-gradient-to-br ${recipeGradient(recipe)} px-5 py-5 mb-4 flex items-center gap-4 pr-20 overflow-hidden`}>
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex items-center gap-1.5 mt-2 flex-wrap">
                <span className="text-sm px-2 py-0.5 rounded-full bg-white/80 text-gray-600 font-medium">
                  ⏱ {recipe.time}
                </span>
                ...
                {(() => {
                  ...
                  return d ? (
                    <span
                      className="text-sm px-2 py-0.5 rounded-full bg-white/90 text-brand-success font-semibold"
                ...
                {recipe.tags?.map((tag) => (
                  <span key={tag} className="text-sm px-2 py-0.5 rounded-full bg-white/60 text-gray-500">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          {matchedItems.length > 0 && (
            <div className="bg-brand-success/5 border border-brand-success/15 rounded-2xl px-3 py-2.5 mb-3">
              <p className="text-sm text-gray-500 mb-1">보유 중인 재료</p>
          ...
          {missingKeywords.length > 0 && (
            <div className="bg-amber-50 border border-amber-100 rounded-2xl px-3 py-2.5 mb-4">
              <div className="flex items-center justify-between mb-1.5">
                <p className="text-sm text-gray-500">부족한 재료 · 탭하면 쇼핑 리스트에 추가</p>
                {missingKeywords.filter((kw) => !inShopping(kw)).length >= 2 && (
                  <button
                    ...
                    className="text-sm font-semibold px-2 py-0.5 rounded-full bg-amber-600 text-white hover:opacity-90 active:scale-95 transition-all"
                  >
                    모두 담기
                  </button>
                )}
          ...
          {(() => {
            ...
            return (
              <div className="rounded-2xl border border-gray-100 bg-gray-50/60 px-3 py-2.5 mb-4">
                <p className="text-sm text-gray-500 mb-1">1인분 영양 추정</p>
                ...
                <p className="text-xs text-gray-400 mt-1 leading-relaxed">재료 카테고리 평균 기반 대략치</p>
              </div>
            );
          })()}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              ...
              <button
                ...
                className="text-sm font-semibold text-gray-500 hover:text-brand-primary hover:underline"
              >
                📋 레시피 복사
              </button>
            </div>
            <ol className="flex flex-col gap-2.5">
              {recipe.steps.map((step, i) => (
                <li key={i} className="flex gap-2.5">
                  <span className="shrink-0 w-5 h-5 rounded-full bg-brand-primary text-white text-sm font-bold flex items-center justify-center">
                    {i + 1}
                  </span>
          ...
          <div className="flex items-center gap-1.5 justify-center mt-2">
            ...
            <label className="text-sm text-gray-500 hover:text-brand-primary cursor-pointer">
              다른 날짜
              <input
                type="date"
